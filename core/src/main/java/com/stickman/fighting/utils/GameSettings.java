...
/**
 * Singleton lưu và đọc cài đặt game qua LibGDX Preferences (tương đương
 * SharedPrefs).
 */
public class GameSettings {
    ...
    private void load() {
        ...
        float rawHpScale = prefs.getFloat(Constants.PREF_HP_SCALE, Constants.HP_SCALE_MIN);
        hpScale = Math.max(Constants.HP_SCALE_MIN, Math.min(Constants.HP_SCALE_MAX, rawHpScale));
    }
    ...
    public void setRoundTime(int t) {
        this.roundTime = t;
    }
    ...
    public void setHpScale(float s) {
        this.hpScale = Math.max(Constants.HP_SCALE_MIN, Math.min(Constants.HP_SCALE_MAX, s));
    }
}
