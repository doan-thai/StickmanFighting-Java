 ...
 * Player 2 (Đỏ):    ←/→ di chuyển, ↑ nhảy, Num4 block, Num5 đấm, Num6 đá, Num3 tốc biến
 */
public class PlayerFighter extends Fighter {

    public enum PlayerIndex { PLAYER_ONE, PLAYER_TWO }
    ...
    protected void updateLogic(float delta) {
        boolean isBlockingNow = Gdx.input.isKeyPressed(keyBlock);
        ...
        if (Gdx.input.isKeyPressed(keyBlock) && Gdx.input.isKeyJustPressed(keyPunch)) {
            energySkill();
            return;
        }

        if (Gdx.input.isKeyJustPressed(keyPunch)) {
            punch();
        }
